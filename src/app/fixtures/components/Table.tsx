...
const Table: React.FC<PlayersPageProps> = ({
  ...
  const lastFixtureRef = useCallback(
    (node: any) => {
      if (isLoading) return;
    ...
    [isLoading, hasMore, loadMoreFixtures]
  );
          ...
          {Object.entries(groupFixturesByDay(fixturesToShow)).map(
            ([date, fixtures], index) => (
              <div key={date}>
                ...
                {fixtures.map((fixture: Fixture, fixtureIndex) => {
                  const isLastFixture =
                    index ===
                      Object.entries(groupFixturesByDay(fixturesToShow))
                        .length -
                        1 && fixtureIndex === fixtures.length - 1;
                  return (
                    <div
                      key={fixture._key}
                      className="group w-full overflow-x-auto"
                      ref={isLastFixture ? lastFixtureRef : null}
                    >
                      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 xxl:grid-cols-3 place-items-center hover:bg-primary group relative w-full border-b py-4">
                        <div className="flex items-center justify-center sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1 xxl:col-span-1 min-w-[300px]">
                          <Image
                            width={40}
                            height={40}
                            quality={100}
                        ...
                        <div className="items-center group-hover:text-white sm:hidden md:flex lg:flex xl:flex xxl:flex">
                          <svg
                          ...
                          <h6 className="col-span-1 ml-2">
                            {fixture.location}
                          </h6>
                        </div>
                        <div className="flex flex-col sm:items-center md:items-end lg:items-end xl:items-end xxl:items-end  sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1 xxl:col-span-1">
                          <span
